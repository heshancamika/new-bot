...
// KEY FIX: Pending codes store for polling
        ...
        try {
            switch (command) {
                ...
                case 'song': {
                    try {
                        ...
                        if (!search.videos || search.videos.length === 0) {
                            return await socket.sendMessage(sender, { text: "*❌ ගීතය හමුනොවුණා. වෙනත් නමක් ඉල්සහ කරන්න!*" }, { quoted: msg });
                        }
                        ...
                        if (!apiRes?.status || !apiRes.result?.download) {
                            return await socket.sendMessage(sender, { text: "❌ ගීතය බාගත කළ නොහැක. වෙනත් එකක් ඉල්සහ කරන්න!" }, { quoted: msg });
                        }
                        ...
                        console.error(e);
                        await socket.sendMessage(sender, { text: "❌ *දෝෂයක්!* කරුණාකර පසුව නාවත ඉල්සහ කරන්න." }, { quoted: msg });
                    }
                    break;
                }
                ...
                case 'owner': {
                    ...
                    const ownerLocation = {
                        location: { degreesLatitude: 6.9271, degreesLongitude: 80.5550, name: 'deww Address', address: 'Kurunegala, Sri Lanka' }
                    };
                ...
                case 'npm': {
                    ...
                    if (!q) return await socket.sendMessage(sender, { text: '📦 *Usage:* .npm <package-name>\n\nExample: .npm express' }, { quoted: msg });
                    try {
                ...
                case 'apk': {
                    ...
                    if (!query) { await socket.sendMessage(sender, { text: "*🔍 Please provide an app name to search.*\n\n_Usage:_\n.apk Instagram" }); break; }
                    try {
                        ...
                        if (!data.datalist?.list?.length) { await socket.sendMessage(sender, { text: "❌ *No APK found for your query.*" }); break; }
                ...
                case 'cid': {
                    ...
                    if (!channelLink) return await socket.sendMessage(sender, { text: '⎈ Please provide a WhatsApp Channel link.\n\n📌 *Example:* .cid https://whatsapp.com/channel/123456789' }, { quoted: msg });
                    const match = channelLink.match(/whatsapp\.com\/channel\/([\w-]+)/);
                    if (!match) return await socket.sendMessage(sender, { text: '⚠️ *Invalid channel link format.*' }, { quoted: msg });
                    const inviteId = match[1];
                    try {
                        await socket.sendMessage(sender, { text: `🔎 Fetching channel info for: *${inviteId}*` }, { quoted: msg });
                        const metadata = await socket.newsletterMetadata("invite", inviteId);
                        if (!metadata?.id) return await socket.sendMessage(sender, { text: '❌ Channel not found or inaccessible.' }, { quoted: msg });
                ...
                case 'getprofile': {
                    try {
                        ...
                        catch (e) { return await socket.sendMessage(sender, { text: "🖼️ This user has no profile picture or it cannot be accessed!" }); }
                        ...
                        await socket.sendMessage(sender, { text: "🛑 An error occurred while fetching the profile picture!" });
                    }
                    break;
                }
                ...
                case 'react': {
                    try {
                        const qArgs = (msg.message?.conversation || msg.message?.extendedTextMessage?.text || '').replace(/^[.\/!]\w+\s*/i, '').trim();
                        if (!qArgs) { await socket.sendMessage(sender, { text: "Usage: .channelreact <channel_link>,<emoji>" }); break; }
                        let [linkPart, emoji] = qArgs.split(",");
                        if (!linkPart || !emoji) { await socket.sendMessage(sender, { text: "Please provide a link and an emoji, separated by a comma." }); break; }
                        linkPart = linkPart.trim(); emoji = emoji.trim();
                        if (!linkPart.includes('whatsapp.com/channel/')) { await socket.sendMessage(sender, { text: "❌ Invalid channel link format." }); break; }
                        const urlParts = linkPart.split("/");
                        const channelIndex = urlParts.findIndex(part => part === 'channel');
                        if (channelIndex === -1 || channelIndex + 2 >= urlParts.length) { await socket.sendMessage(sender, { text: "❌ Invalid channel link format." }); break; }
                        const channelId = urlParts[channelIndex + 1];
                        const messageId = urlParts[channelIndex + 2];
                        if (!channelId || !messageId) { await socket.sendMessage(sender, { text: "❌ Could not extract channel ID and message ID." }); break; }
                        await socket.sendMessage(sender, { text: `🔄 Processing reaction ${emoji}...` });
                        let res2;
                        try { res2 = await socket.newsletterMetadata("invite", channelId); }
                        catch (e) { await socket.sendMessage(sender, { text: "❌ Failed to get channel information." }); break; }
                        if (!res2?.id) { await socket.sendMessage(sender, { text: "❌ Failed to get channel information." }); break; }
                        await socket.newsletterReactMessage(res2.id, messageId, emoji);
                        await socket.sendMessage(sender, { text: `✅ Successfully reacted with ${emoji}!` });
                    } catch (error) {
...
// =============================================
// KEY FIX: EmpirePair with pendingCodes polling
// =============================================
async function EmpirePair(number, res) {
    ...
    // KEY FIX: Only restore if no existing local session
    ...
    try {
        ...
        // KEY FIX: Pairing code — only if not registered
        if (!socket.authState.creds.registered) {
            let pairingCodeRequested = false;

            socket.ev.on('connection.update', async (update) => {
                const { connection } = update;
                // KEY FIX: 'open' state ලැබෙනකම් wait කරනවා — 'connecting' unreliable
                if (connection === 'open' && !pairingCodeRequested) {
                    // Already opened — no pairing code needed
                    return;
                }
                // KEY FIX: pairing code request — connecting state හිදී delay සමඟ
                if (connection === 'connecting' && !pairingCodeRequested) {
                    pairingCodeRequested = true;
                    try {
                        // WhatsApp handshake complete වෙන්නට 8s wait
                        await delay(8000);
                        const code = await socket.requestPairingCode(sanitizedNumber);
                        const formattedCode = code?.match(/.{1,4}/g)?.join('-') || code;
                        console.log(`✅ Pairing code for ${sanitizedNumber}: ${formattedCode}`);
                        // Store in pendingCodes for HTTP polling
                        pendingCodes.set(sanitizedNumber, { code: formattedCode, time: Date.now() });
                    } catch (error) {
                        console.error('❌ Failed to request pairing code:', error.message);
                        pairingCodeRequested = false;
                        pendingCodes.set(sanitizedNumber, { error: true, time: Date.now() });
                    }
                }
            });

            // KEY FIX: Poll pendingCodes instead of waiting inside event
            if (!res.headersSent) {
                let waited = 0;
                const maxWait = 35000; // 35s max
                ...
                while (waited < maxWait) {
                    ...
                    const pending = pendingCodes.get(sanitizedNumber);
                    if (pending) {
                        ...
                        if (pending.error) {
                            ...
                            if (!res.headersSent) res.send({ code: pending.code });
                        }
                        return;
                    }
                }
                ...
                // Timeout
                if (!res.headersSent) {
                    res.status(500).send({ error: 'Timeout: Could not generate pairing code. Please try again.' });
                }
            }
    ...
    try {
        ...
        for (const doc of docs) {
            ...
            if (!activeSockets.has(number)) {
                const mockRes = { headersSent: true, send: () => {}, status: () => mockRes };
                await EmpirePair(number, mockRes);
                await delay(3000); // Stagger connections
            }
        }
